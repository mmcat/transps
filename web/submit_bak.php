...
function output($dir,$name){
        ...
        <div id="title">
        <!-- Name of tool -->
        <h1><a href="index.html"><strong>TransPostScan</strong>: Transcriptome Post Scaffolding. </a></h1>
        ...
        <div class="left">
        <div class="download">
        <a href="TransPostScan1.1.0.tar.gz"><img src="images/download_logo.png" alt="download" width="250"/></a>
        </div>
        <h2>Other data</h2>
        <ul>
        ...
        <h2>Results</h2>
        <table id="table">
        <tr><td><p>some illustration (statistics)...</p></td></tr>
        <tr><td>Accepted Contigs:</td><td>'."<a href=$dir/$name.accept>$name.map </a>".'</td></tr>
...
if(!empty($_FILES["seq_file"]["tmp_name"]) && !empty($_FILES["blastx_file"]["tmp_name"])){
	if($_FILES["seq_file"]["size"] < 20000000 &&  $_FILES["blastx_file"]["size"] < 20000000){
		...
		$cmd = "perl TransPostScan1.1.0/transpostscan.pl -t ".$_FILES["seq_file"]["tmp_name"]." -b ".$_FILES["blastx_file"]["tmp_name"]." --per $per --rate $rate --dist $dist --evalue $evalue";
...
//			mkdir("$dir",0757);
//		}
		mycopy($_FILES["seq_file"]["tmp_name"].".map","$outdir/$name.map");
		mycopy($_FILES["seq_file"]["tmp_name"].".scaffold","$outdir/$name.scaffold"); 
		mycopy($_FILES["seq_file"]["tmp_name"].".accept","$outdir/$name.accept");
		mycopy($_FILES["seq_file"]["tmp_name"].".unused","$outdir/$name.unused");
		output("https://bioinformatics.cs.vt.edu/~mingming/transpostscan/results",$name);
	...
	if($_FILES["blastx_file"]["tmp_name"]<20000000){
		...
		$cmd = "perl TransPostScan1.1.0/transpostscan.pl -t $indir/$name -b ".$_FILES["blastx_file"]["tmp_name"]." --per $per --rate $rate --dist $dist --evalue $evalue";
		if(myexec($cmd)==1){return;}
		mycopy("$indir/$name.map","$outdir/$name.map");
		mycopy("$indir/$name.accept","$outdir/$name.accept");
		mycopy("$indir/$name.scaffold","$outdir/$name.scaffold");
		mycopy("$indir/$name.unused","$outdir/$name.unused");
		output("https://bioinformatics.cs.vt.edu/~mingming/transpostscan/results",$name);
	...
	$name="example.fa";
	$indir="$pwd/results";
	$cmd = "perl TransPostScan1.1.0/transpostscan.pl -t $indir/$name -b $indir/example.blastx --per $per --rate $rate --dist $dist --evalue $evalue";
//	echo $cmd;
	if(myexec($cmd)==1){return;}
	output("https://bioinformatics.cs.vt.edu/~mingming/transpostscan/results",$name);
}
